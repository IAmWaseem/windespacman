...
MisterFrostView::MisterFrostView(MisterFrost * pMisterFrost)
{
...
MisterFrostView::~MisterFrostView(void)
{
}

void MisterFrostView::Draw(HDC hDC, RECT rect, int xFrom, int xTo)
{
	if(!pWorld->Instance()->menu)
	{
		int x = pMisterFrost->GetLocation()->X;
		int y = pMisterFrost->GetLocation()->Y;
		int width = pMisterFrost->GetLocation()->width;
		int height = pMisterFrost->GetLocation()->height;
		...
		if(this->pMisterFrost->GetDirection() == Direction::Left)
